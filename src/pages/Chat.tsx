  ...
  const fetchProviderData = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://fazpramim-back.onrender.com/api/accounts/requests/${id}/`,
        {
          method: "GET",
          headers: {
      ...
      if (requestData.status !== "accepted" && requestData.status !== "completed") {
        if (userType === "cliente") navigate("/solicitacoes-cliente");
        else navigate("/solicitacoes-prestador");
        return;
      }
      ...
      setProviderData({
        id: target?.user?.id || target?.id || 0,
        username: tempName,
        first_name: tempName.split(" ")[0] || "",
        last_name: tempName.split(" ").slice(1).join(" "),
        profile_picture: target?.profile_photo || "/placeholder.svg",
      });
            ...
            <div className="p-4 border-b">
              <div className="flex items-center gap-3">
                <Avatar>
                  ...
                  <AvatarFallback>
                    {providerData?.first_name?.[0]}
                    {providerData?.last_name?.[0]}
                  </AvatarFallback>
                </Avatar>
                <div>
